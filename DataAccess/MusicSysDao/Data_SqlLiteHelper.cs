...
{
    public class Data_SqlLiteHelper
    {
        ...
        public Data_SqlLiteHelper(string dataSourceFilePath, string connectionstring)
        {
            //首先创建相应的SQLLite对象
            if (!File.Exists(dataSourceFilePath))
            {
                System.Data.SQLite.SQLiteConnection.CreateFile(dataSourceFilePath);
            }

            this._SQLiteConnString = connectionstring;
            this._connection = new SQLiteConnection(this._SQLiteConnString);
            this._connection.Commit += new SQLiteCommitHandler(_SQLiteConn_Commit);
            this._connection.RollBack += new EventHandler(_SQLiteConn_RollBack);
        }

        /// <summary>
        /// Data_SqlLiteHelper析构函数
        /// </summary>
        ~Data_SqlLiteHelper()
        {
        ...
        {
            this._connection.BeginTransaction(isoLevel);
            this._IsRunTrans = true;
        }
        /// <summary>
        /// 提交当前挂起的事务
        ...
        public int Execute(string command)
        {
            int result = -1;
            this.Open();
            using (SQLiteCommand sqlitecmd = new SQLiteCommand(command, this._connection))
            {
        ...
        public DataSet GetDs(string command, string tablename, out SQLiteCommand SqlItecmd)
        {
            DataSet ds = new DataSet();
            this.Open();
            SQLiteCommand sqlcmd = new SQLiteCommand(command, this._connection);
